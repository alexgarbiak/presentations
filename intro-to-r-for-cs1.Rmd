...
Let's "*fit a simple linear regression model to a data set*" using the `lm(formula)`{.R} function and the symbolic syntax `response ~ terms`{.R}
...
$y = \alpha + \beta_1X_1 + \beta_2X_2 + \,_{\dots}\, + \beta_nX_n$
...
We also need to be able to "*fit a generalised linear model to a data set*". Here we use the `glm()`{.R} function
...
## Plotting with ggplot2 `r emo::ji("chart increasing")`

```{r ggplot-code, eval=FALSE}
library(ggplot2)
```
