...
- `R` has a number of in-built functions for probability distributions
  - **d***\<distribution-name\>* $:=$ **d**ensity, *i.e.* $\boldsymbol{P}(X>x)$
  - **p***\<distribution-name\>* $:=$ **p**robability distribution function, *i.e.* $\boldsymbol{P}(X \leq x)$
...
| `dbinom(4, size=5, prob=0.5)`{.R} | Computes $\boldsymbol{P}(X = 4)$ where $X \sim \mathcal{B}(5,\,0.5)$ |
